...
static int getMicroSecondTimeout(struct timeval* timeout)
{
  ...
  gettimeofday(&now, 0);
  result = (timeout->tv_sec - now.tv_sec) * 1000000 + timeout->tv_usec - now.tv_usec;
  if (result < 0)
  {
    result = 0;
  }
...
static void notifyCurl(CURL* curl, curl_socket_t s, int evBitmask, const char* info)
{
  int numhandles = 0;
...
int test(char *URL)
{
  ...
  if (!libtest_arg3) {
    fprintf(stderr, "Usage: lib580 [url] [filename] [username]\n");
    return -1;
  }
